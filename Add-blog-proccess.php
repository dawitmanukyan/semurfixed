<?php 
    ...
    && isset($_POST['inforu']) && isset($_POST['infoeng'])){
        ...
        if(!empty($_FILES['file']) && !empty($_FILES['file2']) && !empty($_FILES['file3']) && !empty($_FILES['file4']) && !empty($_FILES['file5'])){
            ...
            if(!move_uploaded_file($file['tmp_name'], $pathFile2)){
                echo 'Տվյալ Ֆայլը չեք կարող ներբեռնել!';
            }
            ...
            if(!move_uploaded_file($file['tmp_name'], $pathFile4)){
                echo 'Տվյալ Ֆայլը չեք կարող ներբեռնել!';
            }

            $pathFile5 = __DIR__.'/img-blog/'.$name5;
            if(!move_uploaded_file($file['tmp_name'], $pathFile5)){
                echo 'Տվյալ Ֆայլը չեք կարող ներբեռնել!';
            }

            $mysql -> query("INSERT INTO `blogs`(`namearm`,`nameru`,`nameeng`,`img`,`img2`,`img3`,`img4`,`img5`,`descriptionarm`,`descriptionru`,`descriptioneng`)
            VALUES ('$catarm','$catru','$cateng','$name','$name2','$name3','$name4','$name5','$infoarm','$inforu','$infoeng')");
